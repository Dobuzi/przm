...
describe("buildRegionCardPresentation", () => {
  ...
  it("uses comparison state before hover and focus", () => {
    const presentation = buildRegionCardPresentation({
      isSelected: false,
      isComparison: true,
      isHovered: true,
    ...
    expect(presentation.badge).toBe("비교 지역");
    expect(presentation.tone).toBe("comparison");
  });
...
describe("buildLegendItems", () => {
  it("returns legend groups for risk, selection, and focus", () => {
    const items = buildLegendItems(true);
    ...
    expect(items).toEqual([
      ...
      { key: "comparison", label: "비교 지역", description: "상세 패널에서 현재 기준과 비교 중인 지역" },
